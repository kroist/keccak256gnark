...
static inline void keccakf(void* state) {
	...
	for (int i = 0; i < 24; i++)
	{
		...
		auto* bb = static_cast<uint8_t*>((void*)a);
		// Rho and pi
		uint64_t t = a[1];
		x = 0;
		REPEAT24(b[0] = a[pi[x]];
...
{
    uint8_t* output;
    output = new uint8_t[32];
	string in = "test";
    const uint8_t* input = reinterpret_cast<const uint8_t*>(in.c_str());
	hash2(output, 32, input, 4, 200 - (256 / 4), 0x01);
	prnt_arr(output);
    cout << endl;
}
